// Run once to seed Firestore with rooms, public areas, and restaurants:
...
if (!serviceAccount) {
  console.log('Using default credentials (dev mode). Make sure emulators are running.');
}
...
// Rooms extracted from Excel: 6xx-26xx (435 rooms total)
const rooms = [
...
async function seed() {
  try {
    ...
    for (const roomNo of rooms) {
      await db.collection('locations').doc(`room-${roomNo}`).set({
        type: 'Rooms',
        name: roomNo,
        display: `Room ${roomNo}`,
      });
    ...
    // Public areas
    console.log(`Adding ${publicAreas.length} public areas...`);
    for (const area of publicAreas) {
      await db.collection('locations').doc(`public-${area.toLowerCase().replace(/\s+/g, '-')}`).set({
        type: 'Public Area',
        name: area,
        display: area,
      });
    ...
    // Restaurants
    console.log(`Adding ${restaurants.length} restaurants...`);
    for (const restaurant of restaurants) {
      await db.collection('locations').doc(`restaurant-${restaurant.toLowerCase().replace(/\s+/g, '-')}`).set({
        type: 'Restaurant',
        name: restaurant,
        display: restaurant,
      });
